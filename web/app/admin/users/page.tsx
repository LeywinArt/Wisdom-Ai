...
export default function AdminUsersPage(){
  ...
  async function createUser(e: React.FormEvent){
    e.preventDefault()
    await fetch('/admin/users', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, email, is_admin: isAdmin }) })
        ...
        {users.map(u=> (
          <div key={u.id} className="p-3 border rounded flex justify-between items-center">
            <div>
              ...
              <div className="text-sm text-gray-600">{u.email} — {u.is_admin ? 'Admin' : 'User'}</div>
            </div>
            <div>
