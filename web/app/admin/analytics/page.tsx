...
export default function AdminAnalyticsPage() {
  ...
  const eventData = engagement?.event_counts ? Object.entries(engagement.event_counts).map(([name, value]) => ({
    name: name.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
    count: value
  })) : []

  const dauData = engagement?.daily_active_users ? Object.entries(engagement.daily_active_users).map(([date, users]) => ({
              ...
              <Card className="bg-white/5 border-white/10 p-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">{engagement?.total_events?.toLocaleString() ?? 0}</p>
                  </div>
                </div>
              </Card>
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">
                      {engagement?.daily_active_users ? Math.round(Object.values(engagement.daily_active_users).reduce((a, b) => a + b, 0) / Object.keys(engagement.daily_active_users).length) : 0}
                    </p>
                  </div>
                </div>
              </Card>
              ...
              <Card className="bg-white/5 border-white/10 p-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">{engagement?.event_counts ? Object.keys(engagement.event_counts).length : 0}</p>
                  </div>
                </div>
              </Card>
            </div>
