...
const FlaskConicalIcon = dynamic(() => import('lucide-react').then(m => m.FlaskConical), { ssr: false })
...
export function Sidebar({ userName = "User", userEmail = "", onNewChat, showNewChatButton = false }: SidebarProps) {
  ...
  const fetchRecentChats = useCallback(async () => {
    console.log('[Sidebar] Fetching recent chats...')
    setLoadingChats(true)
    setChatsError(null)
    
    try {
      const response = await fetch('/api/chat-history?limit=8', {
        cache: 'no-store',
      })
      ...
      console.log('[Sidebar] Fetched chats:', data?.length || 0)
      ...
      console.error('[Sidebar] Error fetching chats:', error)
      setChatsError(error instanceof Error ? error.message : 'Failed to load')
      setRecentChats([])
    } finally {
  ...
  // Fetch on mount and when pathname changes (to refresh after new chats)
  useEffect(() => {
    fetchRecentChats()
  }, [fetchRecentChats, pathname])
  ...
  const truncateQuestion = (question: string, maxLength = 28) => {
  ...
  const adminItems = [
    ...
    { href: '/admin/comparison', icon: FlaskConicalIcon, label: 'RAG Comparison' },
  ]
      ...
      <nav className="flex-1 overflow-y-auto px-4 py-2">
        ...
        {loadingChats ? (
          <div className="flex flex-col items-center justify-center py-6">
            <LoaderIcon className="h-5 w-5 text-orange-400 animate-spin" />
            <p className="text-xs text-white/30 mt-2">Loading chats...</p>
          </div>
        ) : chatsError ? (
          <div className="flex flex-col items-center justify-center py-6 text-center">
            <p className="text-xs text-red-400/70">{chatsError}</p>
            <button 
              onClick={fetchRecentChats}
              className="text-[10px] text-orange-400 mt-2 hover:underline"
            >
              Retry
            </button>
          </div>
          ...
          <div className="space-y-1">
            {recentChats.map((chat) => (
              <Link
                ...
                className="group flex flex-col gap-0.5 rounded-lg px-3 py-2 transition-all duration-200 hover:bg-orange-500/10 hover:pl-4"
              >
                <div className="flex items-center gap-2">
                  <MessageSquareIcon className="h-3 w-3 text-orange-400/60 flex-shrink-0" />
                  <span className="text-xs text-white/70 group-hover:text-white truncate flex-1">
                    {truncateQuestion(chat.question)}
                  </span>
                </div>
                <div className="flex items-center gap-1 pl-5">
                  <ClockIcon className="h-2.5 w-2.5 text-white/20" />
